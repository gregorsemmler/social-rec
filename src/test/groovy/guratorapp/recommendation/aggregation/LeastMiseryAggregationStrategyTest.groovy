...
/**
 * Tests an {@link LeastMiseryAggregationStrategy}
 */
@CompileStatic
class LeastMiseryAggregationStrategyTest extends GroovyAssert{
    ...
    void testAggregateRatings() {
        Collection<GroupRestaurantRating> result = new LeastMiseryAggregationStrategy().aggregateRatings(restaurants, testgroup)

        Assert.assertEquals(10, result.size())

        Assert.assertEquals(6 as long, result[0].restaurantId)
        Assert.assertEquals(5 as long, result[1].restaurantId)
        Assert.assertEquals(4 as long, result[2].restaurantId)
        Assert.assertEquals(8 as long, result[3].restaurantId)
        ...
        Assert.assertEquals(9 as long, result[7].restaurantId)
        Assert.assertEquals(3 as long, result[8].restaurantId)
        Assert.assertEquals(1 as long, result[9].restaurantId)

        Assert.assertEquals(8.0 , result[0].score, DELTA)
        Assert.assertEquals(7.0 , result[1].score, DELTA)
        Assert.assertEquals(6.0 , result[2].score, DELTA)
        Assert.assertEquals(6.0 , result[3].score, DELTA)
