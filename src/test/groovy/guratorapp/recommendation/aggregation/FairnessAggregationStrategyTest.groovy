...
/**
 * Tests an {@link guratorapp.recommendation.aggregation.FairnessAggregationStrategy}
 */
@CompileStatic
class FairnessAggregationStrategyTest extends GroovyAssert{
    ...
    void testAggregateRatings() {
        FairnessAggregationStrategy strategy = new FairnessAggregationStrategy()
        ...
        Assert.assertFalse(firstId == 3 || firstId == 7 || firstId == 8 || firstId == 10)

        Assert.assertEquals(9.0 , result[0].score, DELTA)
        Assert.assertEquals(8.0 , result[1].score, DELTA)
        Assert.assertEquals(7.0 , result[2].score, DELTA)
        Assert.assertEquals(6.0 , result[3].score, DELTA)
        Assert.assertEquals(5.0 , result[4].score, DELTA)
        Assert.assertEquals(4.0 , result[5].score, DELTA)
        Assert.assertEquals(3.0 , result[6].score, DELTA)
        Assert.assertEquals(2.0 , result[7].score, DELTA)
