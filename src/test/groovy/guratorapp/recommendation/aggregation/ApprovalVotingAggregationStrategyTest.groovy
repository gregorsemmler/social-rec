...
/**
 * Tests an {@link guratorapp.recommendation.aggregation.ApprovalVotingAggregationStrategy}
 */
@CompileStatic
class ApprovalVotingAggregationStrategyTest extends GroovyAssert{
    ...
    void testAggregateRatings() {
        ApprovalVotingAggregationStrategy strategy = new ApprovalVotingAggregationStrategy()

        strategy.approvalThreshold = 7.0 as double
        ...
        long middleId = result[2].restaurantId
        long middleId2 = result[8].restaurantId

        Assert.assertTrue(firstId == 5 || firstId == 6)
        Assert.assertTrue(middleId == 1 || middleId == 4 || middleId == 8 || middleId == 9 ||middleId == 10)
        Assert.assertTrue(middleId2 == 2 || middleId2 == 3)
        Assert.assertEquals(7 as long, result[9].restaurantId)

        Assert.assertEquals(3.0 , result[0].score, DELTA)
        Assert.assertEquals(3.0 , result[1].score, DELTA)
        Assert.assertEquals(2.0 , result[2].score, DELTA)
        Assert.assertEquals(2.0 , result[3].score, DELTA)
        Assert.assertEquals(2.0 , result[4].score, DELTA)
        Assert.assertEquals(2.0 , result[5].score, DELTA)
        Assert.assertEquals(2.0 , result[6].score, DELTA)
        Assert.assertEquals(1.0 , result[7].score, DELTA)
        Assert.assertEquals(1.0 , result[8].score, DELTA)
        Assert.assertEquals(0.0 , result[9].score, DELTA)
    }
}
