 ...
 */
@CompileStatic
class SuccessAtNTest extends GroovyAssert {
    ...
    @Test
    void testEvaluate() {
        SuccessAtN successAtN = new SuccessAtN()
        successAtN.N = 3

        double result = new SuccessAtN().evaluate(baselineRatings, evaluateRatings)
        Assert.assertEquals(1.0, result, DELTA)
    }

    @Test
    void testIsSuccess() {
        boolean result = new SuccessAtN().isSuccess(baselineRatings, evaluateRatings)
